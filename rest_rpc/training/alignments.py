...
class Alignments(Resource):
    ...
    def get(self, project_id):
        ...
        if retrieved_alignments:
            ...
            logging.info("Error retrieving alignments", description=f"MFA has not been performed for Project '{project_id}'!", code=404, Class=Alignments.__name__)
            ns_api.abort(
                code=404, 
    ...
    def post(self, project_id):
        ...
        try:
            ...
            for curr_expt in all_expts:
                ...
                # logging.debug(f"Modified input config: {input_config}", Class=Alignments.__name__)
                # logging.debug(f"Modified experiment: {expt_model}", Class=Alignments.__name__)
                ...
                # logging.debug(f"Modified output config: {output_config}", Class=Alignments.__name__)
                # logging.debug(f"Modified experiment: {expt_model}", Class=Alignments.__name__)
                ...
                # logging.debug(f"Updated records: {expt_records.read(**curr_expt['key'])}", Class=Alignments.__name__)
